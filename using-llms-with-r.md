...
Some of these tools also allow for connecting R to so-called open-weight models, which can reduce the cost of LLM use (no subscriptions) and, when run locally, increase data privacy and security (see [Ollama](https://ollama.com/), which can run these models either on your own machine or on its hosted cloud). The use of open-weight models is outside the scope of this tutorial, but the R help on `ellmer` is a good place to start if you wish to investigate how to use open-weight models with R.
