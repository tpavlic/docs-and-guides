...
Provider choice is orthogonal to everything else in this guide. It determines who supplies and bills for the model, not how much the assistant can see of your MATLAB session, and these agents are MCP-capable, so a bring-your-own-key setup still gets the full [section 5](#5-beyond-simple-editing-accessing-the-matlab-session-with-coding-assistants-model-context-protocol) bridge to your live workspace. For anyone with a subsidized or self-hosted model, this is often the cheapest and most privacy-preserving way to drive a capable assistant beside your editor.
...
3. **AI layer.** Layer on whichever assistant you want as an extension. This is the payoff of choosing VS Code: GitHub Copilot, Claude Code, or Codex, each searchable in the Extensions pane. VS Code's own built-in **Chat** is a capable agent in its own right, and it accepts your own key, which makes it the natural companion to the [bring-your-own-key](#bring-your-own-key-byok-provider-agnostic-agents) setups in section 3.
   ...
   This path needs **no GitHub account and no Copilot plan**: once at least one such model is configured, VS Code opens the Chat view and stops prompting you to sign in. The exceptions are inline completions, next-edit suggestions, and semantic search, which still require a GitHub sign-in, since bring-your-own-key covers chat and agent work rather than as-you-type completion. For a student with an institutional key and no Copilot or Claude subscription, this is the shortest route to a full-featured, MCP-capable agent sitting beside a real MATLAB session. On a managed Copilot Business or Enterprise account, though, an administrator can disable the feature by policy.
...
Every flag also has an environment-variable form: uppercase the name, replace hyphens with underscores, and prefix `MW_MCP_SERVER_`, so `--matlab-root` becomes `MW_MCP_SERVER_MATLAB_ROOT`. That is often easier to set in an assistant's configuration file than a command line, and CLI flags win where you set both.
...
| Your setup (exemplar)                                                        | What the MATLAB MCP Server adds                                                                                                     |
| ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
